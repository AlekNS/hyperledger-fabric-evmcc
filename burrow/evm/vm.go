...
import (
	...
	"github.com/JincorTech/hyperledger-fabric-evmcc/burrow/common/sanity"
...
func (vm *VM) call(caller, callee *Account, code, input []byte, value int64, gas *int64) (output []byte, err error) {
	...
	for {
		...
		case MSTORE: // 0x52
			...
			if useGasNegative(gas, GasMStorageLoad, &err) {
				return nil, err
			}
		...
		case MSTORE8: // 0x53
			...
			if useGasNegative(gas, GasMStorageUpdate, &err) {
				return nil, err
			}
		...
		case SLOAD: // 0x54
			loc := stack.Pop()
			if useGasNegative(gas, GasStorageLoad, &err) {
				return nil, err
			}
		...
		case JUMP: // 0x56
			...
			if useGasNegative(gas, GasCodeJump, &err) {
				return nil, err
			}
			continue
		...
		case JUMPI: // 0x57
			...
			if !cond.IsZero() {
				...
				if useGasNegative(gas, GasCodeJumpi, &err) {
					return nil, err
				}
				continue
			}
		...
		case CREATE: // 0xF0
			...
			if err_ != nil {
				...
				vm.appState.UpdateAccount(newAccount) // @NOTE: was added
				vm.appState.UpdateAccount(callee)     // @NOTE: was added
			}
		...
		case CALL, CALLCODE, DELEGATECALL: // 0xF1, 0xF2, 0xF4
			...
			if nativeContract := registeredNativeContracts[addr]; nativeContract != nil {
				...
				if err == nil && value > 0 { // @NOTE: Was added
					vm.appState.UpdateAccount(caller)
					vm.appState.UpdateAccount(callee)
					vm.appState.UpdateAccount(acc)
				}
			}
		...
		case REVERT: // @NOTE: was added
			...
			return output, fmt.Errorf("REVERT")
